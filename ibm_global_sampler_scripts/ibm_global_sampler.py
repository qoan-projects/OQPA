...
def good_bitstrings(k,nqpa):
    """
    Return all bit-strings of length 2k+1 that count as “success”.

    Layout (little-endian → right-most bit is q0):
        | q3 (k bits) | q2 (k bits) | q0 |

    Success rule
    ------------
    • q0 == '0'  →  q2 must be all-zero,  q3 can be anything
    • q0 == '1'  →  q3 must be all-zero,  q2 can be anything
    """
    if nqpa==0:
    ...
    # Case 1: q0 = 1, q3 = 0…0, q2 = anything
    for q2 in product("01", repeat=k):
        accepted.append( zeros_k +"".join(q2) + "1")

    # Case 2: q0 = 0, q2 = 0…0, q3 = anything
    for q3 in product("01", repeat=k):
        accepted.append("".join(q3) + zeros_k + "0")

    return accepted
def from_bitstrings_to_results(k,bitstrings):
    print('Good Bitstrings that lead fidelity:\n')
    for bitstring in bitstrings:
        q3 = bitstring[:k]
        q2 = bitstring[k:2*k]
        q0 = bitstring[2*k:]
        print(f'q0={q0}, q2={q2}, q3={q3}')
...
def get_fidelity_from_paulis(qc_list, shots,k,nqpa,sampler,backend):
    ...
    fid=0
    accepted_bitstrings = good_bitstrings(k,nqpa)
    from_bitstrings_to_results(k,accepted_bitstrings)
...
def main():
    ...
    if aer_testing == True:
        ...
        print(f'Using FakeSherbrooke for testing because fake_testing={fake_testing}')
