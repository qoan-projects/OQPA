...
def get_QPA_circuit(d: int,
    ...
    ----------
    d : int
        Number of data qubits per register (→ total qubits = 4·d).
    n_qpa : int
    ...
    creg = ClassicalRegister(d, "control")
    qreg = QuantumRegister(4 * d, "q")
    qc = QuantumCircuit(qreg, creg)

    # ------------------------------------------------------------------
    # 1) Optional manual depolarisation
    # ------------------------------------------------------------------
    if test_depolarization:
        pauli_names = ["x", "y", "z"]
        rng = np.random.default_rng()  # convenient & reproducible if needed

        for qubit in range(4 * d):
            if rng.random() < eps:
                # Pick gate name, turn it into the bound method QuantumCircuit.x/y/z
                gate_name = rng.choice(pauli_names)
                getattr(qc, gate_name)(qubit)        # <-- this is safe; qc.x etc.

    # ------------------------------------------------------------------
    # 2) QPA swap-net (recursive)
    # ------------------------------------------------------------------
